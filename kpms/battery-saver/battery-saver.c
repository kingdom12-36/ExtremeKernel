...
/*
 * battery-saver.kpm  v2.0  -  ExtremeKernel Battery Saver
 ...
 *   sched_energy_aware        : *   -> 1    (prefer efficient little cores)
 *   vm_swappiness             : *   -> 20   (stay in RAM, avoid swap I/O)
 *   sched_migration_cost_ns   : *   -> 5ms  (reduce inter-cluster migrations)
 */
...
KPM_DESCRIPTION("Max battery life: I/O batching + EAS + low swappiness. Full restore on unload.");
...
{
    pr_info("[ek-battery] Loading ExtremeKernel Battery Saver v2.0...\n");
    ...
    /* Save originals then apply battery values */
    if (p_writeback_interval) {
        orig_writeback_interval = *p_writeback_interval;
        *p_writeback_interval = 3000; /* 30s */
    ...
    if (p_sched_energy_aware) {
        ...
        *p_sched_energy_aware = 1;
        pr_info("[ek-battery] sched_energy_aware:        %u -> 1\n", orig_sched_energy_aware);
    }
    if (p_swappiness) {
    ...
    if (p_migration_cost) {
        ...
        *p_migration_cost = 5000000; /* 5ms — fewer cross-cluster task migrations */
...
static long batt_saver_exit(void *__user reserved)
{
    ...
    if (p_sched_energy_aware) { *p_sched_energy_aware = orig_sched_energy_aware; pr_info("[ek-battery] sched_energy_aware restored:         %u\n", orig_sched_energy_aware); }
